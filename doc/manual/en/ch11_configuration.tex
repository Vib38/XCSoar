...
\item[Waypoints] List of waypoint files. If left blank, waypoints are loaded
  from the map file (if available).

\item[Watched waypoints*] \label{conf:watched-wps} List of waypoint files containing
...
\item[Slope shading*]  \label{conf:shading} The terrain can be shaded to indicate slopes and 
  other terrain features. The illumination direction for the shading can be either: geographically
  fixed from the north-west, screen-relative fixed from the top left, or the shading can be 
  according to the actual current sun position. In addition, the illumination can be set to indicate 
...
\item[Contours*]  Draws contour lines on the terrain.  Two groups of
  modes are available.
  The adaptive modes ({\bf Mountains}, {\bf Highlands}, {\bf Lowlands},
  {\bf Superfine}) set a base line density that is automatically scaled
  with the zoom level to avoid over-cluttering at small scales.
  {\bf Mountains} is for areas with steep slopes, with a 256\,m base
  spacing; the other adaptive modes provide progressively smaller
  spacing and higher line density.
  The fixed modes ({\bf Fixed 256\,m}, {\bf Fixed 128\,m},
  {\bf Fixed 64\,m}) use a constant contour spacing regardless of zoom
  level.
\end{description}
...
See Section~\ref{sec:notam-airspace}.

\begin{description}
\item[NOTAM Support]  Enables downloading and display of NOTAMs from
  aviation authorities.
\item[API URL]  Base URL of the NOTAM proxy API.
\item[Search Radius]  Radius around the current location for which NOTAMs
...
\item[Auto-Refresh (minutes)]  Time interval for automatic NOTAM updates.
  Set this to 0 to disable automatic refresh and use manual refresh only.
\item[Show IFR-Only NOTAMs]  Includes NOTAMs marked as IFR-only traffic.
  This is disabled by default.
\item[Show Only Currently Effective]  Hides NOTAMs which are not effective
  at the current time.  This is enabled by default.
\item[Maximum NOTAM Radius]  Hides NOTAMs whose affected radius is larger
  than this value.  Set this to 0 to disable the radius filter.  The
  default is 100~km.
\item[Hidden Q-Codes]  Space-separated list of Q-code prefixes to hide.
  Prefixes are matched case-insensitively.  The default list is
  \texttt{QA QK QN QOL QOA QOBTT}, hiding administrative, checklist,
...
are currently filtered by each filter.  The \button{Refresh} button saves
the current settings and requests a new download for the current
position.  The \button{List} button opens the current NOTAM list for
review.
...
\begin{description}
\item[Main area]  The primary content of the page, for example \emph{Map}
...
\item[Bottom area]  Optional panel below the main area.  Select
  ...
  controls when the page uses a weather overlay.  Select \emph{Course
  director} to show the left/right deviation band described in
  Section~\ref{sec:course-director}.
...
\item[Key]  Create a key on the page \url{https://skylines.aero/tracking/info} 
...
\item[Max.\ Cruise Speed] Optional upper limit for MacCready speed-to-fly
